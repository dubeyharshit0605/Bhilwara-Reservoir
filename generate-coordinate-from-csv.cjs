...
//   node generate-coordinate-from-csv.cjs ".\src\data\water_data.csv"

const csvPath = process.argv[2];

if (!csvPath) {
  console.error('❌ Please provide path to water_data.csv');
  console.error('   Example: node generate-coordinate-from-csv.cjs ".\\src\\data\\water_data.csv"');
  process.exit(1);
}

if (!fs.existsSync(csvPath)) {
  console.error('❌ CSV file not found at:', csvPath);
  console.error('   Checked path:', csvPath);
  process.exit(1);
}
...
const outRoot = path.resolve(process.cwd(), 'coordinate.json');
const outPublic = path.resolve(process.cwd(), 'public', 'coordinate.json');
const outSrc = path.resolve(process.cwd(), 'src', 'data', 'coordinate.json');
